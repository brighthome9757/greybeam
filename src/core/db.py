...
def execute_query(target_dialect: str, target_queries: dict) -> dict:
    match target_dialect:
      case "postgres":
        target_queries["postgres"] = execute_pg_query(pg_query=target_queries["postgres"])
      case "clickhouse":
        target_queries["clickhouse"] = execute_ch_query(ch_query=target_queries["clickhouse"])
      case _:
        print(f"{target_dialect} is an invalid target database.")

        target_queries[target_dialect]["error"] = f"{target_dialect} is an invalid target database."
        ...
        with psycopg2.connect(**conn_params) as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                ...
                if cursor.description:
                    ...
                    fetch_time_ms = (time.time() - exec_start_time) * 1000
                # Ignore non-SELECT queries
                else:
                    print(f'Only SELECT queries are allowed. Your transpiled query: {pg_query}')
